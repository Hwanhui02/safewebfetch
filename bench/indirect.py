...
if "--codex5" in sys.argv:
    from indirect_codex5 import ATTACKS, BENIGN
elif "--codex4" in sys.argv:
    from indirect_codex4 import ATTACKS, BENIGN
elif "--codex3" in sys.argv:
...
for name, page, bad in ATTACKS:
    r = s.clean(page, guard=guard, judge=judge, nominate="--nominate" in sys.argv)
...
label = (s.GUARD_MODEL.split("/")[-1] if guard else "rules only") + (f" + judge {judge}" if judge else "") + (" + nominate" if "--nominate" in sys.argv else "") + (" | codex5" if "--codex5" in sys.argv else " | codex4" if "--codex4" in sys.argv else " | codex3" if "--codex3" in sys.argv else " | codex2" if "--codex2" in sys.argv else " | codex" if "--codex" in sys.argv else " | holdout2" if "--holdout2" in sys.argv else " | holdout" if "--holdout" in sys.argv else "")
